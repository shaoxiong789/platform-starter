        ...
        <div class="mobile_bd">
          <ul class="pre_menu_list grid_line ui-sortable ui-sortable-disabled no_menu" style="z-index:3;" id="menuList">
              ...
              <div class="sub_pre_menu_box js_l2TitleBox" v-if="index===activeMenuIndex">
                <ul class="sub_pre_menu_list">
                  <li v-for="(sub,index) in btn.sub_button" :key="sub.id" class='jslevel2'>
                    <a href="javascript:void(0);" @click="menu_item_selected(sub.name,index)"
                    :class="[{pre_menu_link: index===activeMenuItemIndex && activeMenuType()==2}]"
                    class="jsSubView" draggable="false">
            ...
            <li class="js_addMenuBox pre_menu_item grid_item no_extra size1of1">
              <a href="javascript:void(0);" v-if="isSet(menu.button)?(menu.button.length>=0&&menu.button.length<3):false" @click="menu_add()" :class="[{pre_menu_link: activeMenuIndex===''}]" class="js_addL1Btn" title="最多添加3个一级菜单" draggable="false"> <i class="icon14_menu_add"></i> <span class="js_addMenuTips">添加菜单</span></a>
            </li>
          </ul>
        </div>
      </div>
          ...
          <div class="menu_form_bd" id="view">
            ...
            <div class="frm_control_group" v-show="((activeMenuIndex > 0 || activeMenuIndex === 0) && (menu.button[activeMenuIndex].sub_button.length == 0)) || (activeMenuItemIndex > 0 || activeMenuItemIndex === 0) " style="display: none;">
              ...
              <div class="frm_controls frm_vertical_pt">
                <label class="frm_radio_label js_radio_sendMsg" @click="radio_label_selected(1)" :class="[{selected:showMenuContentType===1}]" data-editing="0">
                ...
                <label class="frm_radio_label js_radio_url" @click="radio_label_selected(3)" :class="[{selected:showMenuContentType===3}]" data-editing="0">
                  <i class="icon_radio"></i> <span class="lbl_content">点击菜单</span>
                  <input type="radio" name="hello" class="frm_radio" />
                </label>
              </div>
            </div>
            <div class="menu_content_container">
              ...
              <div class="menu_content url jsMain" id="url" v-show="showMenuContentType===3">
                <form action="" id="urlForm" onsubmit="return false;">
                  <p class="menu_content_tips tips_global">用户点击菜单后，开发人员会获得KEY</p>
                  <div class="frm_control_group">
                    <label for="" class="frm_label">key</label>
                    <div class="frm_controls">
                      <span class="frm_input_box">
                        <input style="width: 190px" class="frm_input" type="text" v-if="activeMenuType() == 1 && isSet(menu.button[activeMenuIndex])" v-model="menu.button[activeMenuIndex].key" value="">
                        <input style="width: 190px" class="frm_input" type="text" v-if="activeMenuType() == 2 && isSet(menu.button[activeMenuIndex].sub_button[activeMenuItemIndex])" v-model="menu.button[activeMenuIndex].sub_button[activeMenuItemIndex].key" value="">
                      </span>
                      </p>
                    </div>
                  </div>
...
export default {
  ...
  data() {
    return {
      ...
      "showMenuContentType": '', //1:发送消息 2:跳转链接 3:点击菜单
      ...
      for (var i = 0; i < this.menu.button.length; i++) {
        if (this.menu.button[i].sub_button.length > 0) {
          ...
          for (var j = 0; j < this.menu.button[i].sub_button.length; j++) {
            ...
            } else if (this.menu.button[i].sub_button[j].type == 'media_id') {
          ...
          } else if (this.menu.button[i].type == 'media_id') {
            this.new_menu.button[i] = {
              "type": this.menu.button[i].type,
              "name": this.menu.button[i].name,
              "media_id": this.menu.button[i].media_id
